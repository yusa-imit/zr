...
fn handleListTasks(allocator: std.mem.Allocator, params_json: []const u8) !ToolResult {
    ...
    const exit_code = list.cmdList(
        ...
        false, // group_by_tags = false
        null, // recent_count = null
        null, // frequent_count = null
        null, // slow_threshold_ms = null
        null, // search_description = null
        false, // show_status = false (not relevant for MCP JSON output)
        false, // show_cache = false (not relevant for MCP JSON output)
        false, // show_env = false (not relevant for MCP JSON output)
        ...
        false, // show_source = false (not relevant for MCP JSON output)
        false, // show_last_run_tags = false (not relevant for MCP JSON output)

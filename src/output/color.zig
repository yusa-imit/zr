...
pub fn isTty(file: std.fs.File) bool {
    ...
    // Note: sailor.term.isatty() exists but doesn't handle std.fs.File.handle correctly
    // for cross-compilation (expects posix.fd_t not Windows HANDLE). Keep local impl.

...
fn cmdInit(
    ...
    std.fs.cwd().access(CONFIG_FILE, .{}) catch |err| {
        ...
        // FileNotFound is expected — proceed to create.
        const file = std.fs.cwd().createFile(CONFIG_FILE, .{ .exclusive = true }) catch |cerr| {
            try color.printError(err_writer, use_color,
    ...
    // If access() succeeded, the file already exists.
    try color.printError(err_writer, use_color,
...
test "cmdInit creates zr.toml" {
    const tmp = std.testing.tmpDir(.{});
    // We can't easily call cmdInit against a tmp dir without refactoring,
    // so just verify the template is valid non-empty TOML content.

...
/// Split a `{ ... }, { ... }` value body on top-level `},{` boundaries, respecting `{`/`}`
/// nesting depth, and append each trimmed `{...}` table string to `out`.
/// Extracted from `parseTaskParamsArray` to keep that function under the line-length ratchet;
/// behavior is unchanged (this is the same loop, just callable on its own).
fn splitTopLevelBraceTables(allocator: std.mem.Allocator, params_str: []const u8, out: *std.ArrayList([]const u8)) !void {
    ...
    for (params_str, 0..) |ch, i| {
        ...
        if (ch == '}') {
            ...
            if (depth == 0) {
                ...
                try out.append(allocator, table_str);
                start = i + 1;
            }
        }
    }

    assert(out.items.len >= out_count_before); // Tables are only ever appended.
    assert(depth == 0 or params_str.len > 0); // Unbalanced-depth-on-empty-input is impossible.
}
...
fn parseTaskParamsArray(
    ...
    try splitTopLevelBraceTables(allocator, params_str, &param_tables);
...
pub fn parseToml(allocator: std.mem.Allocator, content: []const u8) !Config {
    ...
    // Tool version constraints (v1.83.0) — non-owning slices for requires pairs
    var task_requires = std.ArrayList([2][]const u8){};
    defer task_requires.deinit(allocator);
        ...
        if (std.mem.startsWith(u8, trimmed, "[[workflows.") and std.mem.endsWith(u8, trimmed, ".stages]]")) {
            ...
            if (std.mem.eql(u8, key, "include") and current_task == null and current_profile == null and current_workflow == null and !in_workspace and !in_metadata and !in_imports and !in_settings and !in_vars and !in_constraint and !in_cache and !in_cache_remote and !in_versioning and !in_conformance and !in_tools and !in_task_env and !in_task_watch and !in_task_matrix and !in_task_hooks and !in_task_retry and !in_task_description and !in_task_outputs and !in_profile_vars and current_plugin_name == null) {
                ...
                if (std.mem.eql(u8, key, "cmd")) {
                    ...
                    // Tool version constraints: { node = ">=18.0.0", python = "^3.8.0" } (v1.83.0)
                    const inner = std.mem.trim(u8, value, " \t");
                    if (std.mem.startsWith(u8, inner, "{") and std.mem.endsWith(u8, inner, "}")) {
                        const pairs_str = inner[1 .. inner.len - 1];
                        var pairs_it = std.mem.splitScalar(u8, pairs_str, ',');
                        while (pairs_it.next()) |pair_str| {
                            const eq = std.mem.indexOf(u8, pair_str, "=") orelse continue;
                            const req_key = std.mem.trim(u8, pair_str[0..eq], " \t\"");
                            const req_val = std.mem.trim(u8, pair_str[eq + 1 ..], " \t\"");
                            if (req_key.len > 0) {
                                try task_requires.append(allocator, .{ req_key, req_val });
                            }
                        }
                    }
                }
                ...
                if (std.mem.eql(u8, key, "cmd")) {
                ...
                } else if (std.mem.eql(u8, key, "retry_max")) {
                    // Flat inline field, documented alongside `retry = {...}` and
                    // `[tasks.X.retry]` (v1.47.0, issue #124: was never wired up).
                    const trimmed_val = std.mem.trim(u8, value, " \t\"");
                    task_retry_max = std.fmt.parseInt(u32, trimmed_val, 10) catch 0;
                } else if (std.mem.eql(u8, key, "retry_delay_ms")) {
                    const trimmed_val = std.mem.trim(u8, value, " \t\"");
                    task_retry_delay_ms = std.fmt.parseInt(u64, trimmed_val, 10) catch 0;
                } else if (std.mem.eql(u8, key, "retry_backoff")) {
                    const trimmed_val = std.mem.trim(u8, value, " \t\"");
                    task_retry_backoff = std.mem.eql(u8, trimmed_val, "true") or std.mem.eql(u8, trimmed_val, "exponential");
                } else if (std.mem.eql(u8, key, "retry_backoff_multiplier")) {
                    const trimmed_val = std.mem.trim(u8, value, " \t\"");
                    task_retry_backoff_multiplier = std.fmt.parseFloat(f64, trimmed_val) catch null;
                } else if (std.mem.eql(u8, key, "retry_jitter")) {
                    const trimmed_val = std.mem.trim(u8, value, " \t\"");
                    task_retry_jitter = std.mem.eql(u8, trimmed_val, "true");
                } else if (std.mem.eql(u8, key, "max_backoff_ms")) {
                    const trimmed_val = std.mem.trim(u8, value, " \t\"");
                    task_max_backoff_ms = std.fmt.parseInt(u64, trimmed_val, 10) catch null;
                } else if (std.mem.eql(u8, key, "retry_on_codes")) {
                    // Parse array: [1, 2, 255]
                    const trimmed_val = std.mem.trim(u8, value, " \t");
                    if (std.mem.startsWith(u8, trimmed_val, "[") and std.mem.endsWith(u8, trimmed_val, "]")) {
                        const codes_str = trimmed_val[1 .. trimmed_val.len - 1];
                        var codes_it = std.mem.splitScalar(u8, codes_str, ',');
                ...
                } else if (std.mem.eql(u8, key, "retry_on_patterns")) {
                    // Parse array: ["Connection refused", "Timeout"]
                    const trimmed_val = std.mem.trim(u8, value, " \t");
                    if (std.mem.startsWith(u8, trimmed_val, "[") and std.mem.endsWith(u8, trimmed_val, "]")) {
                        const patterns_str = trimmed_val[1 .. trimmed_val.len - 1];
                        var patterns_it = std.mem.splitScalar(u8, patterns_str, ',');
                    ...
                    // v1.24.0 (bug fix, issue #124): Inline hooks array, documented in
                    // docs/guides/config-reference.md as the primary syntax but never
                    // wired up — only the `[[tasks.X.hooks]]` array-of-tables form worked.
                    // Parse array: [{ point = "failure", cmd = "..." }, ...]
                    const hooks_trimmed = std.mem.trim(u8, value, " \t");
                    if (std.mem.startsWith(u8, hooks_trimmed, "[") and std.mem.endsWith(u8, hooks_trimmed, "]")) {
                        const hooks_str = hooks_trimmed[1 .. hooks_trimmed.len - 1];
                        // Split by top-level },{ to get individual inline tables
                        var hook_tables = std.ArrayList([]const u8){};
                        defer hook_tables.deinit(allocator);

                        var depth: u32 = 0;
                        var start: usize = 0;
                        for (hooks_str, 0..) |ch, i| {
                            if (ch == '{') depth += 1;
                            if (ch == '}') {
                                depth -= 1;
                                if (depth == 0) {
                        ...
                        for (hook_tables.items) |table_str| {
                            if (!std.mem.startsWith(u8, table_str, "{") or !std.mem.endsWith(u8, table_str, "}")) continue;
                            const inner = table_str[1 .. table_str.len - 1];

                            var hook_cmd: ?[]const u8 = null;
                            var hook_point: ?[]const u8 = null;
                            var hook_failure_strategy: ?[]const u8 = null;
                            var hook_working_dir: ?[]const u8 = null;
                            var hook_env = std.ArrayList([2][]const u8){};
                            defer hook_env.deinit(allocator);

                            var pair_it = std.mem.splitScalar(u8, inner, ',');
                            while (pair_it.next()) |pair_str_raw| {
                                const pair_str = std.mem.trim(u8, pair_str_raw, " \t");
                                if (pair_str.len == 0) continue;
                                const eq2 = std.mem.indexOf(u8, pair_str, "=") orelse continue;
                                const k = std.mem.trim(u8, pair_str[0..eq2], " \t\"");
                                const v = std.mem.trim(u8, pair_str[eq2 + 1 ..], " \t\"");

                                if (std.mem.eql(u8, k, "cmd")) {
                                    hook_cmd = v;
                                } else if (std.mem.eql(u8, k, "point")) {
                                    hook_point = v;
                                } else if (std.mem.eql(u8, k, "failure_strategy")) {
                                    hook_failure_strategy = v;
                                } else if (std.mem.eql(u8, k, "working_dir")) {
                                    hook_working_dir = v;
                                }
                            }

                            try flushCurrentHook(allocator, &task_hooks, hook_cmd, hook_point, hook_failure_strategy, hook_working_dir, &hook_env);
                        }
                    }

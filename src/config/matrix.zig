...
pub fn addMatrixTask(
    ...
    if (dims.items.len == 0) {
        ...
        return addTaskImpl(config, allocator, name, cmd, cwd, description, deps, deps_serial, &[_]types.ConditionalDep{}, &[_][]const u8{}, env, timeout_ms, allow_failure, retry_max, retry_delay_ms, retry_backoff, condition, max_concurrent, cache, max_cpu, max_memory, &[_][]const u8{}, &[_][]const u8{});
    }
    ...
    while (variant_idx < total) : (variant_idx += 1) {
        ...
        try addTaskImpl(config, allocator, vname, v_cmd, v_cwd, v_desc, deps, deps_serial, &[_]types.ConditionalDep{}, &[_][]const u8{}, v_env_list.items, timeout_ms, allow_failure, retry_max, retry_delay_ms, retry_backoff, condition, max_concurrent, cache, max_cpu, max_memory, &[_][]const u8{}, &[_][]const u8{});
    ...
    try addTaskImpl(config, allocator, name, meta_cmd, null, description, variant_names.items, &[_][]const u8{}, &[_]types.ConditionalDep{}, &[_][]const u8{}, &[_][2][]const u8{}, null, false, 0, 0, false, null, 0, false, null, null, &[_][]const u8{}, &[_][]const u8{});
}

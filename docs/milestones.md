...
### Task Template & Matrix Field Parity

Bring dynamically-generated tasks (`[templates.NAME]` expansion via `template = "..."` + `params = {...}`) up to full feature parity with statically-defined `[tasks.NAME]` entries. `expandTemplate()` in `src/config/types.zig` currently hardcodes ~30 Task fields to `null`/empty when building the generated task, silently dropping user config — in some cases (retry advanced options) the parser already parses the template-level value into a local variable and then discards it because `flushCurrentTemplate()`/`TaskTemplate` never carry it through. Includes:
- **Retry field parity**: `retry.backoff_multiplier`, `retry.jitter`, `retry.max_backoff`, `retry.on_codes`, `retry.on_patterns` — parsed for `[templates.NAME]` today but discarded; wire through `TaskTemplate` struct → `flushCurrentTemplate()` → `expandTemplate()`
- **Tagging & organization**: `tags`, `mixins`, `aliases` propagated from template to generated task
- **Up-to-date detection**: `sources`, `generates`, `cache` propagated so templated tasks participate in incremental builds
- **Lifecycle hooks**: `hooks` (before/after/on_error/on_success) propagated
- **Watch integration**: `watch_patterns`, `watch_exclude_patterns`, `watch_debounce_ms`, `watch_mode` propagated
- **Notifications & artifacts**: `notify`/`notify_on`/`notify_title`, `artifacts`/`artifact_retention`/`compress_artifacts` propagated
- **Conditional execution & secrets**: `skip_if`, `output_if`, `deps_if`, `deps_optional`, `required_env`, `secrets`, `input_prompts` propagated
- **Integration/unit tests**: for each newly-wired field, verify a templated task behaves identically to an equivalent hand-written `[tasks.NAME]` task
**Status: ACTIVE** — Established Cycle 452 FEATURE (2026-09-05) per Milestone Establishment Process (technical-debt source; no open feature-request issues at the time). Scoped as incremental sub-slices, one or more fields per cycle, starting with retry field parity (already-parsed-but-discarded data).
...
Enable `zr run --retry-failed` to automatically re-run only the tasks that failed in the previous invocation — essential for large task graphs where flaky tasks or transient errors shouldn't require manually finding and re-running each failed task. After every `zr run`, a failure list is written to `.zr/last-failures.txt`; `--retry-failed` reads this list and re-runs exactly those tasks. Includes:
- **`zr run --retry-failed`** — reads `.zr/last-failures.txt` and re-runs all tasks that failed in the last run; prints "Retrying N failed task(s):" header with task list
- **Automatic persistence**: every `zr run` invocation writes failed task names to `<project>/.zr/last-failures.txt` (one per line); successful runs truncate the file to empty
- **No-op on no failures**: if the file is missing or empty, prints "No previous failures to retry." and exits 0
- **`--retry-failed --dry-run`**: shows which tasks would be retried without executing them; compatible with all existing dry-run output
- **Multiple failed tasks**: comma-joins failed task names and feeds into the existing multi-task run machinery (v1.101.0)
- **Integration tests**: 6 tests (37000–37005) covering no-file case, failure file creation, retry of single/multiple tasks, dry-run, and file clearing on success
**Status: DONE** — Cycle 370 STABILIZATION (2026-06-27). Released as v1.107.0. Implementation: `saveLastFailures()` + `clearLastFailures()` in run.zig (appended after scheduler run), `--retry-failed` branch in main.zig run dispatch (reads `.zr/last-failures.txt`, comma-joins task names, delegates to multi-task run). sailor v2.60.0 (WordCloud widget) also migrated. Testing: 6 integration tests (37000-37005). GitHub release: https://github.com/yusa-imit/zr/releases/tag/v1.107.0

### Task Priority Scheduling

Add a `priority` field (i32) to tasks to control execution order within a topological dependency level. When multiple tasks are ready to run at the same time (same depth in the DAG), higher-priority tasks are scheduled first. Includes:
- **`priority = N`** — integer scheduling priority (positive = earlier, negative = later, default 0)
- **Strict ordering with `--jobs 1`** — single-threaded execution respects priority within each level
- **`zr list` badge** — shows `[p:N]` for non-zero priority tasks
- **`zr list --sort=priority`** — order task list by priority descending (highest first)
- **JSON output** — `priority` field in JSON list output
- **Integration tests**: 6 tests (34000–34005) covering badge display, run ordering, negative priority, --sort=priority, dry-run
**Status: DONE** — Completed Cycle 361 FEATURE (2026-06-22). Released as v1.105.0. Implementation: `priority: i32 = 0` in Task struct (types.zig), TOML integer parsing (parser.zig), `std.mem.sort()` within each topological level in scheduler.zig, badge display and --sort=priority in list.zig, JSON field addition. All addTaskImpl callers updated with `0` default. Testing: 2 unit parser tests, 6 integration tests (34000-34005). sailor v2.55.0 (FlowText) also migrated.
...
Enable tasks to prompt users for runtime inputs — ideal for deployment tasks, version tagging, or any workflow that needs dynamic values each run. Currently users must hardcode values in `[vars]` or pass `--param`, but neither supports interactive prompts. This milestone adds a structured input system with defaults, validation, and CLI override. Includes:
- **`input_prompt = [{name="TAG", prompt="Enter version tag:", default="v1.0.0"}]`** — Task-level input declarations with optional defaults
- **Runtime prompting**: When running interactively, prompt user at task start; values available as `{{TAG}}` template vars in `cmd`, `env`, and other string fields
- **`--input TAG=v1.2.0` CLI flag**: Provide input values non-interactively (bypass prompt); multiple `--input` flags allowed
- **`--non-interactive` flag**: Fail with clear error if required inputs (no default) have no `--input` value provided
- **Dry-run preview**: `zr run --dry-run` shows what would be prompted, with defaults displayed
- **`zr explain` integration**: Show `input_prompt` fields with names, prompts, and defaults
- **Integration with params**: Input values supplement `--param` (same `{{name}}` syntax); if both `--param` and `--input` provide same key, `--input` takes precedence
- **Type hints**: `type = "string"|"bool"|"number"` for basic input validation; `choices = ["prod", "staging", "dev"]` for constrained options
- **Integration tests**: Comprehensive tests covering interactive prompt, --input override, --non-interactive failure, dry-run display, type validation, choices
**Status: DONE** — Completed 2026-06-06 (Cycles 321-323 FEATURE). Implementation: InputPrompt struct + config parser, --input/--non-interactive CLI flags, input collection in cmdRun with validation (type, choices), resolved_params integration with scheduler template substitution, explain text/JSON output, dry-run prompt display. Testing: 13 integration tests (18000-18012). Released as v1.88.0.
...
**Status: DONE** — Completed 2026-06-05 (Cycles 318-321 FEATURE). Implementation: share_output field in Task schema, parser integration with reset fix, ZR_OUTPUT_* env var injection in scheduler, {{output.task-name}} template interpolation, --show-outputs CLI flag, sanitizeTaskNameForEnv() public utility, share_output display in explain (text + JSON). Testing: 21 integration tests (17000-17020). Parser bug fixed (share_output bleeds across tasks). Released as v1.87.0.

### [settings] Section Enhancements

Expand the `[settings]` section (introduced in v1.92.0) with additional project-level configuration defaults. Users can set project defaults that eliminate the need to pass the same CLI flags repeatedly. CLI flags always take precedence over `[settings]` values. Includes:
- **`jobs = N`** — Default `--jobs` value for parallel execution. Overridden by `--jobs` CLI flag. Applied to `run`, `watch`, and `workflow` commands.
...
**Status: DONE** — Released v1.94.0 on 2026-06-14 (Cycle 340 STABILIZATION). Implementation: dot-separated task names via TOML nesting ([tasks.build.compile] → "build.compile"), wildcard group run (zr run build.*), group list filter (zr list --group build), disambiguation for bare group names. Testing: 8 integration tests (23000-23007). sailor v2.38.0 (KeyMap) + v2.39.0 (NumberInput) also included.

### Group-Level Defaults & Inheritance

Extend the Task Groups & Namespaces system (v1.94.0) with namespace-level configuration that all tasks in the group inherit. Large projects often need the same working directory, environment variables, or timeout for all tasks in a group — currently users must repeat these on each task. This milestone adds group configs that eliminate that repetition. Includes:
- **`[tasks.build]` as group config** — a `[tasks.build]` section without a `cmd` field becomes a group config (non-executable); any `build.*` task automatically inherits its `env`, `cwd`, and `timeout`
- **`env` inheritance**: task-level `env` merges with group `env`; task values take precedence on key collision
- **`cwd` inheritance**: task inherits group `cwd` if no task-level `cwd` is set
- **`timeout` inheritance**: task inherits group `timeout` if no task-level `timeout` is set
- **Non-executable group configs**: `zr list` shows group configs with `(group)` indicator; `zr run build` runs tasks in group (not the config itself)
- **Explicit override**: any task field set explicitly on the task overrides the group default
- **`zr explain` integration**: show which fields are inherited from group config vs. set on task
- **Integration tests**: 6 tests (24000–24005) covering env/cwd/timeout inheritance, task override, non-executable group config, explain output
**Status: DONE** — Released v1.95.0 on 2026-06-14 (Cycle 341 FEATURE). Implementation: GroupConfig struct + Config.group_configs map in types.zig; tryAddGroupConfig() in parser.zig detects group configs at flush time (no cmd, no deps, has env/cwd/timeout); applyGroupDefaults() post-processes all tasks for env merge (task wins), cwd fallback, timeout fallback; list.zig shows "(group)" indicator with cwd/env/timeout summary; explain.zig annotates inherited cwd/timeout with source group. Testing: 6 integration tests (24000-24005). GitHub release: https://github.com/yusa-imit/zr/releases/tag/v1.95.0

### Task Dependency Visualization Enhancements

Improve the existing `zr graph` command and dependency visualization with richer output formats and interactive features. Currently users can see a basic ASCII graph, but large projects need better navigation, filtering, and export options. Includes:
- **`zr graph --group <group>`** — filter graph to show only tasks in a specific namespace group and their dependencies
- **`zr graph --depth <n>`** — limit graph traversal depth for large dependency trees
- **`zr graph --from <task>`** — show only the subgraph reachable from a specific task (downstream dependencies)
- **`zr graph --to <task>`** — show only the subgraph that leads to a specific task (upstream dependencies)
- **`zr graph --format dot`** — output GraphViz DOT format for external visualization
- **`zr graph --format mermaid`** — output Mermaid diagram syntax for markdown embedding
- **Cycle detection in output**: highlight cycles with a clear indicator when `--cycles-only` flag is used
- **Integration tests**: 8 tests (25000–25007) covering depth limiting, upstream/downstream filtering, dot/mermaid format output, group filtering
**Status: DONE** — Released v1.96.0 on 2026-06-14 (Cycle 343 FEATURE). Implementation: collectDownstream() BFS via reverse dep map, collectUpstream() BFS via forward deps, detectCycles() DFS, renderTasksMermaid() Mermaid flowchart, renderTasksDot() GraphViz DOT, group/from/to/depth/cycles-only CLI flags in graphCommand, main.zig use_new_graph detection updated to include all new flags. Testing: 8 integration tests (25000-25007). GitHub release: https://github.com/yusa-imit/zr/releases/tag/v1.96.0

### Shell Completion Scripts

Generate shell completion scripts for bash, zsh, and fish so users get tab-completion for `zr` commands, task names, flags, and options. Reduces friction for daily use and makes zr feel like a first-class CLI tool. Includes:
- **`zr completion bash`** — output bash completion script (source in `.bashrc`)
- **`zr completion zsh`** — output zsh completion script (add to `$fpath`)
- **`zr completion fish`** — output fish completion script (save to `~/.config/fish/completions/`)
- **Dynamic task name completion** — completions read `zr.toml` in cwd for task names
- **Flag completion** — all `zr run`, `zr list`, `zr graph`, `zr explain`, etc. flags auto-completed
...
**Status: DONE** — Completed Cycle 350 STABILIZATION (2026-06-16). Released as v1.99.0. Implementation: `include = [...]` top-level key parsed in parser.zig feeding the [imports] machinery; `source_file` field added to Task for tracking origin; `zr list --source` shows [filename] badge; `zr validate --show-includes` shows include tree; JSON list includes `source_file` field; all conflict/merge/override handled via existing mergeConfigs. Testing: 8 integration tests (28000-28007). sailor v2.46.0 (Spinner) + v2.47.0 (DiffStat) also migrated. GitHub release: https://github.com/yusa-imit/zr/releases/tag/v1.99.0

### Run-Level Lifecycle Hooks

Add project-level lifecycle hooks via `[settings]` that run automatically at key points in every `zr run` invocation. Unlike task-level hooks (which apply per-task), these run-level hooks control the overall execution session — essential for CI/CD pipelines, environment setup/teardown, and centralized error handling. Includes:
- **`before_all = ["setup", "check-tools"]`** — tasks run BEFORE any main task; if any fail, the entire run is aborted without running main tasks
- **`after_all = ["cleanup"]`** — tasks ALWAYS run after everything, even when main tasks failed; ideal for teardown, log collection
- **`on_error = ["alert", "rollback"]`** — tasks run ONLY when the main run had at least one failure; ideal for error notifications, rollback actions
- **`on_success = ["notify"]`** — tasks run ONLY when all main tasks succeeded; ideal for deployment notifications, artifact publishing
- **Order**: `before_all` → main tasks → `on_success`/`on_error` → `after_all`
- **`--dry-run` preview**: shows "Run lifecycle hooks:" section listing configured hook tasks by phase
- **Inherits scheduler settings**: hook tasks use the same `--jobs`, `default_timeout`, and color settings as the main run
...
**Status: DONE** — Completed Cycle 357 FEATURE (2026-06-21). Implementation: `--env KEY=VALUE` flag parsing in main.zig (same pattern as `--input`); `cli_env` parameter added to `cmdRun()` + all callers; `cli_env` merged into `extra_env_list` before scheduler call; dry-run shows "CLI env overrides:" section; `required_env` + `secrets` validation in scheduler.zig now checks `extra_env` (includes cli_env). Testing: 6 integration tests (32000-32005). Released as v1.103.0.

### Task Run Tags: Per-Invocation Tag Injection

Allow users to inject task tags at runtime via `--add-tag TAG` so they can group, filter, and label runs dynamically without modifying `zr.toml`. Essential for CI/CD pipelines where the same task needs different labels (environment name, PR number, build ID) and for observability when reviewing history. Includes:
- **`--add-tag TAG`** — attach one or more tags to all tasks in the current run; these are stored in history alongside existing task tags
- **Multiple `--add-tag` flags** — `zr run build --add-tag ci --add-tag pr-123` stacks tags
- **`--skip-tag TAG`** enhancement** — existing `--tag`/`--skip-tag` filtering continues to work; runtime-injected tags are invisible to these filters (they apply to task-defined tags only)
- **`zr history`** integration — history entries show runtime-injected tags alongside task-defined ones (displayed with a `+` prefix: `+ci`, `+pr-123`)
- **`--dry-run` display** — show a "Runtime tags:" section listing all `--add-tag` values when present
- **`zr list --last-run-tags`** — show runtime tags from most recent history entry per task
- **Integration tests**: 6 tests (33000–33005) covering basic tag injection, multiple tags, history recording, dry-run display, list integration, interaction with --tag filter
**Status: DONE** — Cycle 358 FEATURE (2026-06-21). Released as v1.104.0. Implementation: --add-tag FLAG parsing in main.zig, runtime_tags passed through cmdRun/cmdWatch, history recording with tags (tab-separated column), dry-run display of "Runtime tags:" section, --last-run-tags flag on zr list showing +tag annotations per task. Integration tests: 33000-33007 (8 tests).
...
**Status: DONE** — Completed 2026-06-02 (Cycles 304-309 FEATURE). Implementation: ~180 LOC (explain.zig: collectTaskDeps(), printTaskText() with timeout/env/required_env/skip_if/cache/sources display, printDependencyTree() recursive tree, printTaskJson(), cmdExplain() with history-based duration estimates). Testing: 21 integration tests (15000-15020) covering text/tree/json formats, multi-task plans, dependencies, estimated durations, per-field display, error cases. History-based per-task and total estimated duration shown when history available. All features: --tree, --json, --explain flag on run, multiple tasks, cache integration. Ready for v1.85.0 release.
...
| v2.14.0-v2.34.0 | DONE | Batch migration — UI improvements, performance, new widgets (Cycles 293-329) |
| v2.35.0 | DONE | LogViewer widget — log pane TUI integration (Cycle 334, commit 7b6e0a8) |
| v2.36.0 | DONE | FilterBar widget — multi-tag filter bar with pill rendering (Cycle 336, no zr integration needed) |
...
| v2.46.0 | DONE | Spinner widget — animated loading indicator with customizable frames (Cycle 350, v1.99.0) |
| v2.47.0 | DONE | DiffStat widget — colored diff statistics display (Cycle 350, v1.99.0) |
| v2.48.0 | DONE | Marquee widget — scrolling text ticker (Cycle 352, v1.100.0) |
| v2.49.0 | DONE | Wizard widget — multi-step wizard flow (Cycle 352, v1.100.0) |
| v2.50.0 | DONE | Carousel widget — horizontal sliding panel (Cycle 352, v1.100.0) |
| v2.51.0 | DONE | CountdownTimer widget — countdown display with callbacks (Cycle 352, v1.100.0) |
...
| v2.60.0 | DONE | WordCloud widget — Archimedean spiral layout, weight-based styling (Cycle 370 STABILIZATION, v1.107.0, issue #105) |
| v2.61.0 | DONE | KanbanBoard widget — multi-column task board, priority levels (critical/high/normal/low), card navigation (Cycle 371 FEATURE, v1.107.0, issue #106) |
| v2.62.0 | DONE | BracketViewer widget — tournament bracket display, round columns, winner highlighting, score display (Cycle 373 FEATURE, v1.108.0, issue #107) |
| v2.63.0 | DONE | ActivityFeed widget — scrollable timestamped event feed with kind-based icons and color coding (Cycle 374 FEATURE, v1.109.0, issue #108) |

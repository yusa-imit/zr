...
test "lsp: provides completion for task names" {
    ...
    // Line 5 is "deps = [""]", position at character 8 (inside the quotes)
    var completion_buf: [512]u8 = undefined;
    const completion_json = try std.fmt.bufPrint(&completion_buf,
        "{{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"textDocument/completion\",\"params\":{{\"textDocument\":{{\"uri\":\"{s}\"}},\"position\":{{\"line\":5,\"character\":8}}}}}}",
        .{file_uri},
    );

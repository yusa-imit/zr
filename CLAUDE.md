...
- [ ] **v1.34.0 — Workflow Retry Budget Integration**: Complete workflow retry budget implementation in scheduler. Extract retry_budget from workflow config when executing workflow stages, track budget across all stages, enforce workflow-level retry limits, add integration tests for workflow retry scenarios, document workflow retry budget behavior.

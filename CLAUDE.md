...
- [ ] **v1.31.0 — Checkpoint/Resume for Long-Running Tasks**: IN PROGRESS (2026-03-13) — Checkpoint storage infrastructure (CheckpointStorage interface, FileSystemStorage backend), task stdout monitoring for "CHECKPOINT: <data>" markers, resume protocol via ZR_CHECKPOINT env var, scheduler integration. Phase 1 complete (commit 535c514). Tests: 818/826 unit (8 skipped, 0 leaks), 937/937 integration (936 passed, 1 skipped).
- [ ] **v1.32.0 — Sailor v1.11.0 & v1.12.0 Migration**: Upgrade to sailor v1.11.0 (terminal graphics: particle effects, blur/transparency, Sixel/Kitty protocol, animated transitions) and v1.12.0 (enterprise features: session recording/playback, audit logging, WCAG AAA themes, screen reader enhancements). Optional TUI enhancements with new graphics capabilities.
- [ ] **v1.33.0 — Advanced TUI Data Visualization**: Integrate sailor v1.6.0/v1.7.0 data visualization widgets into TUI modes. Histogram for task duration distributions, TimeSeriesChart for build time trends, ScatterPlot for cache performance analysis, FlexBox layout for responsive dashboards, viewport clipping for efficient large graph rendering.

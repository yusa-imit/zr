...
- [ ] **v1.28.0 — Interactive TUI with Mouse Support**: Leverage sailor v1.10.0 mouse input features (mouse click support for task selection in TUI mode, clickable graph nodes for task details, drag-to-scroll in long lists, double-click to execute tasks, context menu on right-click, mouse hover tooltips for task status), integration tests with MockTerminal mouse events, documentation
